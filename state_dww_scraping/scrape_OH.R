oh_base = 'http://dww.epa.ohio.gov/JSP/SearchDispatch?number=&name=&county=All&WaterSystemType=All&SourceWaterType=All&PointOfContactType=None&SampleType=null&begin_date=9%2F12%2F2016&end_date=9%2F12%2F2018&action=Search+For+Water+Systems'
system_base = "http://dww.epa.ohio.gov/JSP/"
library(rvest)
library(tidyverse)
ta = oh_base %>% read_html()
...
tryCatch(base_table <- ta %>% html_nodes('table') %>% html_table(trim=T,fill=TRUE),
         error=function(e) print("OH site appears to have changed."))
...
saveRDS(pws_list,paste0('input/state_dww_database_records/OH_',Sys.Date(),'_PWS.RDS'))
