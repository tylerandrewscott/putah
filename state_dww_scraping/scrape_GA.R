ga_base = 'http://gadrinkingwater.net/DWWPUB/JSP/SearchDispatch?number=&name=&county=All&WaterSystemType=C&SourceWaterType=All&PointOfContactType=None&SampleType=null&begin_date=9%2F1%2F2016&end_date=9%2F1%2F2018&action=Search+For+Water+Systems'
library(rvest)
library(tidyverse)
ta = ga_base %>% read_html()
...
tryCatch(base_table <- ta %>% html_nodes('table') %>% html_table(trim=T,fill=TRUE),
         error=function(e) print("GA site appears to have changed."))
...
system_base = "http://gadrinkingwater.net/DWWPUB/JSP/"
...
saveRDS(pws_list,paste0('input/state_dww_database_records/GA_',Sys.Date(),'_PWS.RDS'))
