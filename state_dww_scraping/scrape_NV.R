nv_base = 'https://ndwis.ndep.nv.gov/DWW/JSP/SearchDispatch?number=&name=&county=All&WaterSystemType=All&SourceWaterType=All&PointOfContactType=None&SampleType=null&begin_date=8%2F30%2F2016&end_date=8%2F30%2F2018&action=Search+For+Water+Systems'
system_base = "https://ndwis.ndep.nv.gov/DWW/JSP/"
library(rvest)
library(tidyverse)
ta = nv_base %>% read_html()
...
tryCatch(base_table <- ta %>% html_nodes('table') %>% html_table(trim=T,fill=TRUE),
         error=function(e) print("NV site appears to have changed."))
...
#                                       html_nodes(css_operators) %>% html_table(trim=T) %>% .[[1]] %>% 
#                                       mutate(PWS_ID = base_table$PWS_ID[x])))
...
pws_list = list(pws_master_list = base_table,
                ...
                service_connections = service_connections_df,operators = NULL,
     services = service_areas_df, 
     water_sources = system_sources_df,purchasing_connections = system_purchases_df)

saveRDS(pws_list,paste0('input/state_dww_database_records/NV_',Sys.Date(),'_PWS.RDS'))
