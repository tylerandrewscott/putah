il_base = 'http://water.epa.state.il.us/dww/JSP/SearchDispatch?number=&name=&county=All&WaterSystemType=All&SourceWaterType=All&PointOfContactType=None&SampleType=null&begin_date=8%2F28%2F2016&end_date=8%2F28%2F2018&action=Search+For+Water+Systems'
library(rvest)
library(tidyverse)
ta = il_base %>% read_html()
...
tryCatch(base_table <- ta %>% html_nodes('table') %>% html_table(trim=T,fill=TRUE),
         error=function(e) print("IL site appears to have changed."))
...
system_base = "http://water.epa.state.il.us/dww/JSP/"
...
saveRDS(pws_list,paste0('input/state_dww_database_records/IL_',Sys.Date(),'_PWS.RDS'))
