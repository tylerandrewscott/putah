de_base = 'https://drinkingwater.dhss.delaware.gov/JSP/SearchDispatch?number=&name=&county=All&WaterSystemType=All&SourceWaterType=All&PointOfContactType=None&SampleType=null&begin_date=9%2F12%2F2016&end_date=9%2F12%2F2018&action=Search+For+Water+Systems'
system_base = "https://drinkingwater.dhss.delaware.gov/JSP/"
library(rvest)
library(tidyverse)
ta = de_base %>% read_html()
...
tryCatch(base_table <- ta %>% html_nodes('table') %>% html_table(trim=T,fill=TRUE),
         error=function(e) print("DE site appears to have changed."))
...
saveRDS(pws_list,paste0('input/state_dww_database_records/DE_',Sys.Date(),'_PWS.RDS'))
