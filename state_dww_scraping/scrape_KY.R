ky_base = 'https://dep.gateway.ky.gov/DWW/JSP/SearchDispatch?number=&name=&county=All&WaterSystemType=All&SourceWaterType=All&PointOfContactType=None&SampleType=null&begin_date=8%2F30%2F2016&end_date=8%2F30%2F2018&action=Search+For+Water+Systems'
system_base = "https://dep.gateway.ky.gov/DWW/JSP/"
library(rvest)
library(tidyverse)
ta = ky_base %>% read_html()
...
tryCatch(base_table <- ta %>% html_nodes('table') %>% html_table(trim=T,fill=TRUE),
         error=function(e) print("KY site appears to have changed."))
...
saveRDS(pws_list,paste0('input/state_dww_database_records/KY_',Sys.Date(),'_PWS.RDS'))
